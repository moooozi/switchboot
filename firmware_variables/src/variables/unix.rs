...
fn efivar_path(name: &str, namespace: &str) -> PathBuf {
    PathBuf::from(format!(
        ...
        name, namespace.trim_matches(|c| c == '{' || c == '}')
    ))
}
...
pub fn set_variable(
    ...
    verify_uefi_firmware().map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "Unsupported firmware"))?;
...
pub fn get_variable(name: &str, namespace: &str) -> Result<(Vec<u8>, Attributes), std::io::Error> {
    ...
    if data.len() < 4 {
        return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "efivar file too short"));
    }
    let attributes = u32::from_le_bytes([data[0], data[1], data[2], data[3]]);
    Ok((data[4..].to_vec(), Attributes::from_bits_truncate(attributes)))
}
