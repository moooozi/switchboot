...
async function calculateMenuPosition(
  ...
  preferredPlacement: 'bottom-start' | 'top-start' | 'right-start' | 'left-start' = 'bottom-start'
): Promise<{ x: number; y: number }> {
  // Create a temporary menu element to measure dimensions
  const tempMenu = document.createElement('div');
  tempMenu.className = 'fixed w-max flex flex-col bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-600 rounded-lg shadow-lg py-2 z-50 opacity-0 pointer-events-none';
  ...
  items.forEach(item => {
    ...
    itemElement.className = 'w-auto text-left px-4 py-2 text-neutral-900 dark:text-neutral-100 transition-colors hover:bg-neutral-100 dark:hover:bg-neutral-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent';
...
export async function openContextMenu(options: {
  triggerElement: HTMLElement;
  items: ContextMenuState['items'];
  preferredPlacement?: 'bottom-start' | 'top-start' | 'right-start' | 'left-start';
  onclose?: () => void;
  owner?: string;
}) {
  const { triggerElement, items, preferredPlacement = 'bottom-start', onclose = () => {}, owner } = options;

  const { x, y } = await calculateMenuPosition(triggerElement, items, preferredPlacement);
...
export function closeContextMenu() {
  contextMenuStore.update(state => ({ ...state, show: false, owner: undefined }));
}
...
export async function toggleContextMenu(
  ...
  const { triggerElement, items, preferredPlacement = 'bottom-start', onclose = () => {} } = options;

  // Get current state
  let currentState: ContextMenuState = { show: false, x: 0, y: 0, owner: undefined, items: [], onclose: () => {} };
